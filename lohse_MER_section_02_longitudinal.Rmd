...
1. If you really, really want p-values for individual parameters, you can get them from packages that implement the Satterthwaite approximation or Kenward-Roger approximation to degrees of freedom, like the "lmerTest"" package.
...
Note if we want to control the type of deviance we calculate, can control that using Anova() function from the car package:

* Type I = sequential (terms are tested in the order they appear in the model)

* Type II = marginal effects (deviance is always assigned to main effect before interactions)

* Type III = parallel (deviance is estimated in parallel, i.e., each term is "controlling for" all other terms in the model

However, note that the Anova() function uses chi-squared tests by default, not F-tests. These are different test statistics and while they generally yield similar conclusions, they will not necessarily yield the same conclusions.

```{r}
anova(cond_01)
# Compare to Anova output
# Type II - marginal
Anova(cond_01, type="II")
# Type III - parallel
...
McCoach, D. B., Rifenbark, G. G., Newton, S. D., Li, X., Kooken, J., Yomtov, D., ... & Bellara, A. (2018). Does the package matter? A comparison of five common multilevel modeling software packages. Journal of Educational and Behavioral Statistics, 43(5), 594-627. https://doi.org/10.3102/1076998618776348 
