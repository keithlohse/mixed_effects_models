...
# NOTE (2024 revision): this file was previously read from "data_example.csv", which
# no longer exists in the repository and returns a 404 error. "data_AGING_example.csv"
# has the identical column structure (subID, group, age_group, condition, time, speed)
# and is confirmed to work with all of the code below.
...
* **Data missing on covariates** (unlike every other row in this table, this is a point of *agreement* between the two methods, not a contrast -- we call that out explicitly here so it doesn't read like a copy-paste error)
    - **RM ANOVA**:
    ...
    - **Mixed-Effects Regression**:
        *	Missing between-person covariate data cannot be accommodated either.  
        *	As with RM ANOVA, cases are either dropped from analysis or retained by imputing missing values (e.g., via multiple imputation) *before* fitting the mixed-effects model -- the mixed-effects machinery itself does not solve this problem for you.
...
Lohse, K., Kozlowski, A., & Strube, M. J. (2023). Model Specification in Mixed-Effects Models: A Focus on Random Effects. Communications in Kinesiology, 1(5). doi:  https://doi.org/10.51224/cik.2023.52  
