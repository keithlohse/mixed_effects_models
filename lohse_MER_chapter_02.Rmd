...
will agree. By default, R also used treatment coding (or "dummy" coding) for categorical
...
our original full data frame, DATA. First, let's plot the data to get a better 
...
produce the same f-observed for the main-effects of time, F(2,158) = 676.1, p < 0.001,
and Condition, F(2, 158) = 68.9, p < 0.001. 
...
# Mixed-Factorial ANOVA with One Crossed Factor 
## (Split-Plot Design)
...
ggplot(data_COND, aes(x = condition, y = speed)) +
  geom_point(aes(fill=group), pch=21, size=2,
             position=position_jitterdodge(dodge.width = 0.5))+
  geom_boxplot(aes(fill=group), col="black", outlier.shape = "na",
               alpha=0.4, width=0.5)+
...
providing Maulchy's Test for Sphericity and both the Greenhouse-Geisser and Hyun-Feldt
...
# Mixed-Factorial ANOVA with Multiple Crossed Factors 
...
Age Group and treatment Group0. As before, we can directly code this into our analysis
of variance using the aov() function or using the ezANOVA() function from the "ez" 
package. 
...
As illustrated in the figure, these results show main-effects of...
