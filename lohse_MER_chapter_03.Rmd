...
Taking our unconditional cubic model as the "base" model, we can now add in condition effects. The model building strategy you use will likely depend on the context of your research question. For instance, in the previous chapter on experimental designs, we knew that we always wanted to test the fully factorial model from the beginning. In this case, however, I will adopt the strategy of building our models a piece at a time. Adding interactions with the low-level factors first and progressively adding in higher-order interactions. 
...
With our three conditional models in place, we can now use the anova() function to formally compare between models. The AIC is now **increasing** as our models get more complex, indicating that the model with linear interactions between Time and AIS_grade is the most parsimonious explanation of our data. 
```{r}
...
Personally, I like to err on the side of completeness, so I would probably present "cond_03" in the text but report the AIC's for each model, making it clear that "cond_01" was the best fittign model. 
