...
``` {r plotting the effects of time, echo=FALSE}
...
In summary, we have the following terms to explain our *DATA*:
    ...
    * Fixed-Effects are the group-level $\beta$'s, these effects parallel the traditional main-effects and interactions that you have probably encountered in other statistical analyses.
    ...
    * The *ERRORS*, or more specifically Random Errors, are the difference between our *MODEL*'s predictions and the actual *DATA*, $\epsilon_{ij}$'s.


# Advantages of Mixed-Effects Regression:
* **Model concept**
    - **RM ANOVA**:
